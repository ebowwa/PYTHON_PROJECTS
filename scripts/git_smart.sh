...
# Function to handle encryption of private files
handle_private_files() {
    local private_path="/Users/ebowwa/PYTHON_PROJECTS/private"
    local recipient=$(git config user.email)
    
    if [ -z "$recipient" ]; then
        echo -e "${RED}Error: Git user email not configured. Please run:${NC}"
        echo "git config --global user.email 'your@email.com'"
        exit 1
    fi
    
    # Check if GPG is installed and keys are set up
    ...
    if ! gpg --list-secret-keys --keyid-format LONG | grep -q "sec"; then
        echo -e "${RED}No GPG keys found. Please run encrypt_repo_files.sh check first${NC}"
        exit 1
    fi
    
    # Find and encrypt modified files in private directory
    echo_step "Checking for sensitive files in private directory..."
    git diff --cached --name-only | grep "^private/" | while read -r file; do
        if [ -f "$file" ] && [[ ! "$file" == *.gpg ]]; then
            echo -e "${GREEN}Encrypting: $file${NC}"
            gpg --output "${file}.gpg" --encrypt --recipient "$recipient" "$file"
            git add "${file}.gpg"
            git reset "$file"  # Remove the original file from staging
        fi
    done
}
    ...
    if [ ! -z "$untracked" ]; then
        ...
        echo "$untracked" | while read -r file; do
            if [ -f "$file" ]; then
                ...
                if [[ "$file" == private/* ]] && [[ ! "$file" == *.gpg ]]; then
                    local recipient=$(git config user.email)
                    echo -e "${GREEN}Encrypting new file: $file${NC}"
                    gpg --output "${file}.gpg" --encrypt --recipient "$recipient" "$file"
                    git add "${file}.gpg"
                else
                    git add "$file"
...
# Handle encryption of private files before committing
handle_private_files
